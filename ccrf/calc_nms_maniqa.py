...
df.to_csv(f'res2/nms_maniqa.csv', index=False)
df = pd.DataFrame([], columns=['path', 'median', 'lower_b', 'upper_b'])
df.to_csv(f'res2/nms_dn_maniqa.csv', index=False)
...
for path in tqdm(paths):
  ...
  df = pd.read_csv(f'res2/nms_maniqa.csv')
  ...
  df = pd.read_csv(f'res2/nms_dn_maniqa.csv')
  df2 = pd.DataFrame([[path, detections[0].item(), detections_l[0].item(), detections_u[0].item()]], columns=['path', 'median', 'lower_b', 'upper_b'])
  df = pd.concat([df2, df])
  df.to_csv(f'res2/nms_dn_maniqa.csv', index=False)
