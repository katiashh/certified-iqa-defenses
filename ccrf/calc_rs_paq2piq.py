...
class MetricClassifier(nn.Module):
    def __init__(self):
      super().__init__()
      self.model = pyiqa.create_metric('paq2piq', device=device)
      self.diap = 84.1711196899414 - 58.3803939819336
...
df.to_csv(f'res2/rs_paq2piq.csv', index=False)
df = pd.DataFrame([], columns=['path', 'pred', 'radius'])
df.to_csv(f'res2/rs_dn_paq2piq.csv', index=False)
...
for preset in presets:
        for attack in attacks:
                path = os.path.join('../../../../data/DIONE/work/Framework_Datasets/dataset/attacked-dataset/no-defence/', preset, attack, 'paq2piq')
...
for path in tqdm(paths):
  ...
  df = pd.read_csv(f'res2/rs_paq2piq.csv')
  df2 = pd.DataFrame([[path, pred, radius]], columns=['path', 'pred', 'radius'])
  df = pd.concat([df2, df])
  df.to_csv(f'res2/rs_paq2piq.csv', index=False)
  ...
  #im = torch.nn.functional.interpolate(im, (256, 256), mode='bicubic', antialias=True)
  pred, radius = certify_clf(x=im[None, :], n=1000, batch_size=10, dn=False)
  df = pd.read_csv(f'res2/rs_dn_paq2piq.csv')
  df2 = pd.DataFrame([[path, pred, radius]], columns=['path', 'pred', 'radius'])
  df = pd.concat([df2, df])
  df.to_csv(f'res2/rs_dn_paq2piq.csv', index=False)
